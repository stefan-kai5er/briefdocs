...
import AssemblyViewer from "../../../../components/AssemblyViewer.tsx"
...
<AssemblyViewer modelUrl={"/models/support-vermeiden/45-grad.glb"} allowFloorClip={true} client:visible/>
...
<AssemblyViewer modelUrl={"/models/support-vermeiden/ueberhang-vergleich.glb"} allowFloorClip={true} client:visible/>

### Bauteil aufteilen
*   **Problem:** Das Bauteil ist zu komplex, um es in einer einzigen, supportfreien Orientierung zu drucken.
*   **Lösung:** Zerlege das Modell in mehrere Stücke. Jedes Teil kann nun in seiner optimalen, Ausrichtung gedruckt und später geklebt oder verschraubt werden (siehe [Gewinde in 3D-Druckteilen](/3d-druck/how-to/gewinde/)). Dies führt oftmals auch zu stabileren Teilen.

<AssemblyViewer modelUrl={"/models/support-vermeiden/bauteil-aufteilen.glb"} allowFloorClip={true} client:visible/>
---
...
<AssemblyViewer modelUrl={"/models/support-vermeiden/supporttypen.glb"} allowFloorClip={true} client:visible/>
...
<AssemblyViewer modelUrl={"/models/support-vermeiden/opfer-layer.glb"} allowFloorClip={true} client:visible/>

---

## Fazit & Empfehlung

Für alle geteilten oder professionell gefertigten Designs ist die vollständige Vermeidung von Slicer-Supports der beste Weg zu konsistenten Ergebnissen. Man eliminiert dadurch die Fehlerquelle der falschen Nutzereinstellungen völlig.

Wenn Supports unvermeidbar sind, maximieren eigens konstruierte CAD-Supports (wie Opfer-Layer, zerdrückbare Stützen oder Support-Pins mit 0.2 mm Spalt) die Zuverlässigkeit und Oberflächenqualität. Auto-generierte Slicer-Supports bleiben der Notnagel: Sie sind der schnellste, aber unzuverlässigste Weg und sollten idealerweise nur für private, schnelle Prototypen genutzt werden, bei denen die CAD-Optimierung zu lange dauern würde.
